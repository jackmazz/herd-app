...
import defaultBannerPhoto from "../assets/field.jpg";
...
import { StatusDropDown } from "components/settings-page/dropdowns/StatusDropDown.jsx";
...
const [numPollOptions, setNumPollOptions] = useState([]);
    ...
    const activeUserToken = sessionStorage.getItem("user-token");
...
const [showAllInterests, setShowAllInterests] = useState(false);
    ...
    const [profileLinkFeedback, setProfileLinkFeedback] = useState("");
    ...
    const handleCloseBioPopup = useCallback(() => {
        setIsEditBioPopupOpen(false);
    }, []);

    const handleCloseBackgroundPhotoPopup = useCallback(() => {
        setIsUploadBackgroundPhotoPopupOpen(false);
    }, []);

    const handleCloseProfilePhotoPopup = useCallback(() => {
        setIsUploadProfilePhotoPopupOpen(false);
    }, []);

    const handleCloseFollowerList = useCallback(() => {
        setIsFollowerListOpen(false);
    }, []);

    const handleCloseFollowingList = useCallback(() => {
        setIsFollowingListOpen(false);
    }, []);

    const handleCloseBlockingConfirmation = useCallback(() => {
        setIsBlockingConfirmationOpen(false);
    }, []);

    const handleCopyProfileLink = async (event) => {
        event.preventDefault();
        ...
        try {
            ...
            setProfileLinkFeedback(t("profilePage.messages.linkCopied"));
        } catch (error) {
            console.error("Failed to copy profile link:", error);
            setProfileLinkFeedback(t("profilePage.messages.linkCopyFailed"));
        }
    ...
    const getUser = () => {
        ...
        return fetch(`${process.env.REACT_APP_API_PATH}/users/${userId}`)
            ...
            .then(data => { 
                if (!data || typeof data !== "object" || !data.id || !data.attributes) {
                    setIsUserNotFound(true);
                    setUser(null);
                    return;
                }
    ...
    if (isHidden) {
        return (
            <div className="block-page-content">
                ...
                <div className="block-page-heading">{t("profilePage.private.title")}</div>
                <button className={isFollowRequested
 ? t("profilePage.private.followRequested")
 : t("profilePage.private.requestFollow")}
  ...
  return (
    ...
    <div className="profile-page-content">
        <div className="profile-page-header">
          <div className="profile-page-header-left">
                            ...
                            {isEditingScreenName ? (
                                <div>
                                    ...
                                    {screenName.length >= screenNameCharLimit && (
                                        ...
                                        {t("profilePage.constraints.maxCharacters", {
                                            count: screenNameCharLimit
                                        })}
                                        </span>
                                    )}
                                </div>
                            ) : (
                                <p className="profile-page-screen-name">{user ? user.attributes.screenname : t("profilePage.loading")}</p>
                            )}
                            {isOwner && !isEditingScreenName &&
                                <button className="editUsernameBtn" onClick={editScreenName}>
                                    <img src={pencilEdit} className="editUsernameIcon" alt="edit-screen-name"/>
                                </button>
                            }

                        </div>
                        <div className="profile-page-username-row">
                            <p className="profile-page-username">@{user ? user.attributes.username : "..."}</p>
                            <div className="profile-link-copy-wrapper">
                                <button
                                    className="profile-link-copy-button"
                                    onClick={handleCopyProfileLink}
                                    title={t("profilePage.buttons.copyProfileLink")}
                                    aria-label={t("profilePage.buttons.copyProfileLink")}
                                >
                                    <img src={linkIcon} alt="copy-profile-link" />
                                </button>
                                {profileLinkFeedback && (
                                    <div className="profile-link-copy-popup" role="status" aria-live="polite">
                                        {profileLinkFeedback}
                                    </div>
                                )}
                            </div>
                            <div className="profileBtns">
                                {!isOwner &&
                                    <button className={isFollowing ? "user-list-item__unfollow-button" : "user-list-item__follow-button"}
                                        onClick={(e) => isFollowing ? unfollowUser() : followUser(e)}
                                        disabled={isFollowProcessing}>
                                        {isFollowing
                                            ? t("profilePage.buttons.following")
                                            : t("profilePage.buttons.follow")}
                                    </button>
                                }
                                {!isOwner && !blockingUser && (
                                    <button className="red-block-button" onClick={blockUser}>
                                        {t("profilePage.buttons.block")}
                                    </button>
                                )}
                            </div>
                        </div>
              ...
              <div className="profile-photo-container">
                  <img src={profilePhoto || defaultProfileIcon} className="profile-photo" alt={t("profilePage.accessibility.profilePhoto")}/>
                  {isOwner &&
                      <button className="profile-photo-button" onClick={editProfilePhoto} aria-label={t("profilePage.buttons.editProfilePhoto")}>
                          <img src={cameraIcon} alt="Profile photo camera icon"/>
                      </button>
                  }
              </div>
              { isUploadProfilePhotoPopupOpen && (
                <ChangeProfilePhotoPopup
                    onClose={handleCloseProfilePhotoPopup}
                    onSave={handleSaveProfilePhoto}
                />
              )}
          </div>
            <img src={backgroundPhoto || defaultBannerPhoto} className="profile-page-background-image" alt={t("profilePage.accessibility.backgroundPhoto")}/>
            {isOwner &&
                <button className="profile-background-button" onClick={editProfileBackground} aria-label={t("profilePage.buttons.editBackgroundPhoto")}>
                    <img src={paintingIcon} alt="Profile background painting icon"/>
                </button>
            }
          {isUploadBackgroundPhotoPopupOpen && (
            <ChangeBackgroundPhotoPopup
                onClose={handleCloseBackgroundPhotoPopup}
      ...
      <p className="profile-page-paragraph">
          {user ? user.attributes.bio || "" : t("profilePage.loading")}
      </p>
        {isEditBioPopupOpen && (
            <EditBioPopup
                onClose={handleCloseBioPopup}
        ...
        <StatusDropDown 
            ...
            isEditable={isOwner}
            isProfile={true}
            showAlert={(message) => console.error(message)}
        />
        <div className="profile-page-heading">
            {t("profilePage.sections.recentCommunities")}
        </div>

{sortedRecentCommunities.length === 0 ? (
    <NoResultsBubble>
        {t("profilePage.empty.noRecentCommunities")}
    </NoResultsBubble>
) : (
    <>
        ...
        {sortedRecentCommunities.length > COMMUNITY_PREVIEW_COUNT && (
            ...
            >
                {showAllCommunities
                    ? t("profilePage.buttons.showLess")
                    : t("profilePage.buttons.showMore")}
            </button>
        )}
    </>
)}
...
{allInterestTags.length === 0 ? (
    <NoResultsBubble>
        {t("profilePage.empty.noInterests")}
    </NoResultsBubble>
) : (
    <>
        <div className="profile-page-community-list">
        ...
        <div className="profile-page-heading">
            {t("profilePage.sections.recentPosts")}
        </div>
        {(!posts || posts.length === 0) ? (
    <NoResultsBubble>
        {t("profilePage.empty.noRecentPosts")}
    </NoResultsBubble>
) : (
        ...
        {posts.map((post) => {
            ...
            return (
                <PostCard
                    ...
                    showFollow={false}
                    onPostUpdated={getRecentPosts}
                    onPostDeleted={() => getRecentPosts()}
                />
            );
        })}
    </div>
        ...
        {isBlockingConfirmationOpen && (
          ...
          <div className="block-page-content">
              ...
              <div className="block-page-heading">
                {t("profilePage.blocking.youBlocking", {
                    username: user?.attributes?.username || "..."
                })}
              </div>
              <button className="popup-ok-btn" onClick={handleUnblockUser}>
                  {t("profilePage.buttons.unblock")}
              </button>
          </div>
          }
          {blockedByUser &&
              <div className="block-page-content">
                  ...
                  <div className="block-page-heading">
                    {t("profilePage.blocking.blockedBy", {
                        username: user?.attributes?.username || "..."
                    })}
                  </div>
              </div>
