...
export const ScreennameField = ({
    ...
    const submit = async () => {      
        ...
        try {
            ...
            showAlert(
                t("settings.errors.updateDisplayName"),
                true
            );
        }
    };
    ...
    const onValidate = useCallback((controller, isActive) => {
        ...
        if (trimmedValue.length === 0) {
            if (isActive()) {
                setValid(false);
                setErrorMessage(t("settings.errors.displayNameRequired"));
            }
            return;
        }
        ...
        if (trimmedValue.length < valueMinLength) {
            if (isActive()) {
                setValid(false);
                setErrorMessage(
                    t("settings.errors.displayNameMin", { count: valueMinLength })
                );
            }
            return;
        }
        ...
        if (trimmedValue.length > valueMaxLength) {
            if (isActive()) {
                setValid(false);
                setErrorMessage(
                    t("settings.errors.displayNameMax", { count: valueMaxLength })
                );
            }
            return;
        }
