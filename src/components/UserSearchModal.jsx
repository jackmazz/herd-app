...
export const UserSearchModal = ({
    ...
    const searchUsers = useCallback(async (controller, isActive) => {    
        ...
        // show an alert
            ...
            if (isActive()) {
                showAlert(t("userSearchModal.alerts.searchFailed"));
            } else {
                return;
            }
        }
    ...
    return (
        <div className="user-search-modal" role="dialog" aria-modal="true" aria-labelledby="user-search-title">
            <div className="user-search-modal__menu" ref={modalRef}>
                <button
                    className="user-search-modal__close-button-wrapper"
                    aria-label={t("userSearchModal.alt.close")}
                    onClick={() => {
                        setSearchValue("");
                        setUserList([]);
                        onClose();
                ...
                <div className="user-search-modal__message" id="user-search-title">
                    {t("userSearchModal.title")}
                </div>
            
                <UserSearchBar
                    placeholder={t("userSearchModal.searchPlaceholder")}
    ...
    searchValue.length === 0 ? (
        <div className="user-search-modal__empty-message">
          {t("userSearchModal.empty.noQuery")}
        </div>
    ) : (
        <div className="user-search-modal__empty-message">
          {t("userSearchModal.empty.loading")}
        </div>
    )
    ...
    searchValue.length === 0 ? (
        ...
        <div className="user-search-modal__empty-message">
          {t("userSearchModal.empty.noResults")}
        </div>
    )
...
const UserListItem = ({
    ...
    const submitFollowRequest = async () => {        
        ...
        // show an alert
            ...
            showAlert(t("userSearchModal.alerts.followFailed"));
        }
    };
    ...
    const submitUnfollowRequest = async () => {
        ...
        // show an alert
            ...
            showAlert(t("userSearchModal.alerts.unfollowFailed"));
        }
    };
    ...
    const requestFollow = async (targetUser) => {
        ...
        try {
            ...
            showAlert(t("userSearchModal.alerts.requestFailed"));
        }
    }
        ...
        <div className="user-list-item">
            <Link 
                ...
                <img 
                    ...
                    alt={t("userSearchModal.alt.profile")}
                />
            ...
            {listedUser.attributes.privateAccount === true ? (
                <button 
                    ...
                    >{isFollowing ? (
                        t("userSearchModal.buttons.unfollow")
                    ) : isFollowRequested ? (
                        t("userSearchModal.buttons.requested")
                    ) : (
                        t("userSearchModal.buttons.request")
                    )}
                </button>
            ) : (
                <button 
                    ...
                    >{isFollowing
                        ? t("userSearchModal.buttons.unfollow")
                        : t("userSearchModal.buttons.follow")}
                </button>
            )}
        </div>
    );
